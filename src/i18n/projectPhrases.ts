...
const descriptions: Record<SupportedLanguage, Record<string, string>> = {
  de: {
    'AI-Assisted Engineering Agent': 'Eine oeffentliche Zusammenfassung eines AWS-Developer-Tooling-Projekts fuer KI-gestuetzte Engineering-Automatisierung. Das System koordiniert Aufgabenaufnahme, Codegenerierung, Code-Review-Lifecycle, Zuverlaessigkeitspruefungen und operative Observability mit Fokus auf datenschutzbewusste Datenverarbeitung, Validierung und Production-Readiness-Guardrails.',
  ...
  fr: {
    'AI-Assisted Engineering Agent': 'Un resume public d un projet d outillage developpeur chez AWS consacre a l automatisation d ingenierie assistee par IA. Le systeme coordonne la prise en charge des taches, la generation de code, le cycle de revue, les controles de fiabilite et l observabilite operationnelle, avec des garde-fous autour de la confidentialite, de la validation et de la preparation a la production.',
  ...
  pl: {
    'AI-Assisted Engineering Agent': 'Publiczne podsumowanie projektu narzedzi developerskich AWS dotyczacego automatyzacji inzynierskiej wspieranej przez AI. System koordynuje przyjmowanie zadan, generowanie kodu, cykl code review, kontrole niezawodnosci i observability, z naciskiem na prywatnosc danych, walidacje i gotowosc produkcyjna.',
...
const aiAgentCaseStudyCopy: Record<SupportedLanguage, Record<string, string>> = {
  de: {
    ...
    'This project explores how an AI-assisted engineering workflow can move from a loosely scoped engineering request toward review-ready software work without removing the human engineer from the loop. The goal is not to replace judgment; it is to reduce repetitive coordination, preserve context, and make each step easier to inspect.': 'Dieses Projekt zeigt, wie ein KI-gestuetzter Engineering-Workflow von einer grob umrissenen Anfrage zu review-faehiger Softwarearbeit fuehren kann, ohne den menschlichen Engineer aus dem Prozess zu nehmen. Ziel ist nicht, Urteilskraft zu ersetzen, sondern wiederholte Koordination zu reduzieren, Kontext zu bewahren und jeden Schritt leichter pruefbar zu machen.',
    ...
    'AI-assisted loop': 'KI-gestuetzte Schleife',
    ...
    'Added reliability and observability hooks so failures could be understood instead of hidden behind the AI layer.': 'Reliability- und Observability-Hooks ergaenzt, damit Fehler verstanden werden koennen, statt hinter der KI-Schicht verborgen zu bleiben.',
    'Why it matters': 'Warum es wichtig ist',
    'AI tools are only useful in serious engineering environments when they fit into the systems engineers already trust. This project treats AI as one component inside a larger delivery workflow, with human review, validation, and operational clarity as first-class requirements.': 'KI-Tools sind in ernsthaften Engineering-Umgebungen nur nuetzlich, wenn sie in die Systeme passen, denen Engineers bereits vertrauen. Dieses Projekt behandelt KI als eine Komponente in einem groesseren Delivery-Workflow, in dem menschliches Review, Validierung und operative Klarheit zentrale Anforderungen sind.',
    'Public-safe takeaway': 'Oeffentlich formulierbare Erkenntnis',
    'The interesting engineering challenge is less about asking a model to write code and more about designing the surrounding system: how context is gathered, how plans become changes, how those changes are checked, and how people stay in control of the final decision.': 'Die interessante Engineering-Herausforderung liegt weniger darin, ein Modell Code schreiben zu lassen, sondern im Design des umgebenden Systems: wie Kontext gesammelt wird, wie Plaene zu Aenderungen werden, wie diese Aenderungen geprueft werden und wie Menschen die finale Entscheidung kontrollieren.',
  },
  fr: {
    ...
    'This project explores how an AI-assisted engineering workflow can move from a loosely scoped engineering request toward review-ready software work without removing the human engineer from the loop. The goal is not to replace judgment; it is to reduce repetitive coordination, preserve context, and make each step easier to inspect.': 'Ce projet montre comment un workflow d ingenierie assiste par IA peut passer d une demande d ingenierie encore peu precise a un travail logiciel pret pour revue sans retirer l ingenieur humain de la boucle. Le but n est pas de remplacer le jugement, mais de reduire la coordination repetitive, preserver le contexte et rendre chaque etape plus facile a inspecter.',
    ...
    'AI-assisted loop': 'Boucle assistee par IA',
    ...
    'Added reliability and observability hooks so failures could be understood instead of hidden behind the AI layer.': 'Ajout de points de fiabilite et d observabilite pour comprendre les echecs au lieu de les masquer derriere la couche IA.',
    'Why it matters': 'Pourquoi c est important',
    'AI tools are only useful in serious engineering environments when they fit into the systems engineers already trust. This project treats AI as one component inside a larger delivery workflow, with human review, validation, and operational clarity as first-class requirements.': 'Les outils IA ne sont utiles dans des environnements d ingenierie serieux que s ils s integrent aux systemes auxquels les ingenieurs font deja confiance. Ce projet traite l IA comme un composant d un workflow de livraison plus large, ou revue humaine, validation et clarte operationnelle sont essentielles.',
    'Public-safe takeaway': 'Conclusion publique',
    'The interesting engineering challenge is less about asking a model to write code and more about designing the surrounding system: how context is gathered, how plans become changes, how those changes are checked, and how people stay in control of the final decision.': 'Le defi d ingenierie interessant consiste moins a demander a un modele d ecrire du code qu a concevoir le systeme autour : comment le contexte est recueilli, comment les plans deviennent des changements, comment ces changements sont verifies et comment les personnes gardent le controle de la decision finale.',
  },
  pl: {
    ...
    'This project explores how an AI-assisted engineering workflow can move from a loosely scoped engineering request toward review-ready software work without removing the human engineer from the loop. The goal is not to replace judgment; it is to reduce repetitive coordination, preserve context, and make each step easier to inspect.': 'Ten projekt pokazuje, jak workflow inzynierski wspierany przez AI moze przejsc od luzno okreslonej prosby inzynierskiej do pracy gotowej do review bez usuwania czlowieka z procesu. Celem nie jest zastapienie oceny, lecz ograniczenie powtarzalnej koordynacji, zachowanie kontekstu i ulatwienie inspekcji kazdego kroku.',
    ...
    'AI-assisted loop': 'Petla wspierana przez AI',
    ...
    'Added reliability and observability hooks so failures could be understood instead of hidden behind the AI layer.': 'Dodanie mechanizmow reliability i observability, aby awarie byly zrozumiale zamiast ukryte za warstwa AI.',
    'Why it matters': 'Dlaczego to ma znaczenie',
    'AI tools are only useful in serious engineering environments when they fit into the systems engineers already trust. This project treats AI as one component inside a larger delivery workflow, with human review, validation, and operational clarity as first-class requirements.': 'Narzedzia AI sa uzyteczne w powaznych srodowiskach inzynierskich tylko wtedy, gdy pasuja do systemow, ktorym inzynierowie juz ufaja. Ten projekt traktuje AI jako jeden komponent wiekszego workflow dostarczania, z review czlowieka, walidacja i przejrzystoscia operacyjna jako kluczowymi wymaganiami.',
    'Public-safe takeaway': 'Publiczny wniosek',
    'The interesting engineering challenge is less about asking a model to write code and more about designing the surrounding system: how context is gathered, how plans become changes, how those changes are checked, and how people stay in control of the final decision.': 'Ciekawe wyzwanie inzynierskie polega mniej na proszeniu modelu o napisanie kodu, a bardziej na zaprojektowaniu systemu dookola: jak zbierany jest kontekst, jak plany staja sie zmianami, jak te zmiany sa sprawdzane i jak ludzie zachowuja kontrole nad ostateczna decyzja.',
  },
}
