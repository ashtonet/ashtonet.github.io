...
import { ArrowUpRight, Code2, FileText, Link2, Mail, Phone, UserRound } from 'lucide-react'

const contacts = [
  { icon: Mail, label: 'Email', value: 'aethom@umich.edu', href: 'mailto:aethom@umich.edu' },
  { icon: Phone, label: 'Phone', value: '(231) 492-8156', href: 'tel:+12314928156' },
...
export default function Contact() {
  return <section id="contact" className="section border-t border-white/[.05]"><div className="shell">
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{contacts.map(({ icon: Icon, label, value, href }) => <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined} rel={href.startsWith('http') ? 'noreferrer' : undefined} className="glass card group flex items-center gap-4 p-4"><div className="icon-box"><Icon size={18} /></div><div className="min-w-0"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 truncate text-sm text-slate-200">{value}</div></div><ArrowUpRight size={14} className="ml-auto shrink-0 text-slate-600 transition group-hover:text-white" /></a>)}</div>
